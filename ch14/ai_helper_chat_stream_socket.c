...
static void socket_stream_cb(const char *chunk, void *user) {
    ...
    write(cfd, chunk, strlen(chunk));    // your code here
}
...
int main(void) {
    ...
    // ---------- 1) TCP 서버 소켓 생성 ----------
    int sfd = socket(AF_INET, SOCK_STREAM, 0);    // your code here
    if (sfd < 0) {
        perror("socket");
        exit(1);
    }
    ...
    struct sockaddr_in addr, cli;    // your code here
    // 클라이언트 주소 길이 변수 선언 및 초기화
    socklen_t cli_len = sizeof(cli);    // your code here

    // memset을 이용한 구조체 초기화
    memset(&addr, 0, sizeof(addr));    // your code here
    // 주소 체계: IPv4 설정
    addr.sin_family = AF_INET;    // your code here
    // 모든 인터페이스에서 수신
    addr.sin_addr.s_addr = htonl(INADDR_ANY);    // your code here
    // 포트 번호 설정
    addr.sin_port = htons(MY_PORT);    // your code here

    // 소켓을 IP/포트에 바인딩. 서버만 bind 필요
    if (bind(sfd, (struct sockaddr*)&addr, sizeof(addr)) < 0) {         // your code here
        perror("bind");
        exit(1);
    }

    // 클라이언트 접속을 받을 준비 (블로킹 아님). 최대 8개 대기 큐
    if (listen(sfd, 8) < 0) {         // your code here
        perror("listen");
        exit(1);
    }
    ...
    while (1) {
        ...
        int cfd = accept(sfd, (struct sockaddr*)&cli, &cli_len);    // your code here   
        if (cfd < 0) {
            perror("accept");
            continue;
        }
        ...
        while (1) {
            ...
            ssize_t n = read(cfd, prompt, sizeof(prompt)-1);    // your code here
            if (n <= 0) break;
            ...
            write(cfd, "<<<END>>>", 9);    // your code here
        }

        printf("[AI Helper] Client disconnected\n");
        // ---------- 7) 클라이언트 소켓 종료 ----------
        close(cfd);    // your code here
    }

    close_ai_context_file();
    // 서버 소켓 종료
    close(sfd);    // your code here
    return 0;
}
