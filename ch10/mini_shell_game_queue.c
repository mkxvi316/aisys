...
void enqueue_job(double dmg) {
    ...
    pthread_cond_signal(&not_empty);                // your code here
    // 큐를 보호하기 위한 뮤텍스 잠금 해제
    pthread_mutex_unlock(&queue_mutex);             // your code here
}
...
job_t* dequeue_job(void) {
    ...
    while (queue_len == 0 && running)
        ...
        pthread_cond_wait(&not_empty, &queue_mutex);        // your code here
    ...
    if (j) {
        ...
        pthread_cond_signal(&not_full);         // your code here
    }
    // 큐를 보호하기 위한 뮤텍스 잠금 해제
    pthread_mutex_unlock(&queue_mutex);         // your code here
    return j;
...
void* worker_main(void* arg) {
    ...
    while (1) {
        ...
        if (pthread_mutex_timedlock(&hp_mutex, &ts) != 0) {        // your code here
        ...
        pthread_mutex_unlock(&hp_mutex);         // your code here
...
int main(void) {
    ...
    pthread_t workers[MAX_WORKER_THREADS];            // your code here      
        ...
        if (ai_mode) {
            if (strcmp(argv[0], "attack") == 0) {
                ...
                for (int i = 1; argv[i] != NULL && i < MAXARGS; i++) {
                    ...
                    if (dmg <= 0) continue;
                    // 큐에 공격 구조체를 추가
                    enqueue_job(dmg);          // your code here
                    has_arg = 1;
                }
                if (!has_arg)
    ...
    pthread_mutex_lock(&queue_mutex);   // your code here
    // 비어있는 워커 깨움
    pthread_cond_broadcast(&not_empty); // your code here
    // 꽉 찬 워커 깨움
    pthread_cond_broadcast(&not_full);  // your code here

    // 큐 뮤텍스 잠금 해제
    pthread_mutex_unlock(&queue_mutex); // your code here

    // 모든 워커 스레드 종료 대기
    for (int i = 0; i < MAX_WORKER_THREADS; i++) {          // your code here
