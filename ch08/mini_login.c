...
int main(void) {
    ...
    if (!user) { // 신규 사용자 생성
        ...
        // [채워진 부분 3] 신규 사용자 생성 후, longjmp를 이용해 로그인 프롬프트 처음으로 돌아갑니다.
        longjmp(env, 1);
    }
    ...
    if (strcmp(res, user->hash) == 0) {
        ...
        // [채워진 부분 5] HOME 환경변수를 현재 디렉터리로 설정합니다.
        char cwd[1024];
        ...
        // [채워진 부분 6] mini_shell_adv를 실행합니다.
        execlp("./mini_shell_adv", "mini_shell_adv", (char *)NULL);
        ...
        printf("Login failed.\n");
        // [채워진 부분 7] 로그인 실패 시 시도 횟수를 증가시키고 longjmp로 재시도합니다.
        login_attempts++;
        if (login_attempts >= 3) {
            printf("Too many failed attemps");
        }
        longjmp(env, 1);
    }
}