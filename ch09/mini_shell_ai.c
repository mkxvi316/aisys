...
void handle_sigquit(int signo) {
    ...
    ai_mode = !ai_mode;                                                 // ai mode 토글
    write(STDOUT_FILENO, ai_mode ? msg_on : msg_off,
          ai_mode ? sizeof(msg_on) - 1 : sizeof(msg_off) - 1);          // ai mode 에 따라서 위의 상태 메시지 출력
}
...
void handle_sigint(int signo) {
    ...
    if (ai_mode && ai_thinking) {           // AI 모드이고, AI가 생각 중일 때
        ai_thinking = 0;
...
void setup_terminal(struct termios *orig) {
    ...
    tcgetattr(STDIN_FILENO, orig);          // 원래 터미널 속성 저장
    new_term = *orig;
    ...
    tcsetattr(STDIN_FILENO, TCSAFLUSH, &new_term);      // tcsetattr 을 이용하여 터미널에 키 설정 적용
}
